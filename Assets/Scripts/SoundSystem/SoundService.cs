...
namespace Assets.Scripts.SoundSystem
{
	...
	public class SoundService : MonoBehaviour
	{
		...
		[Space(10f)] [SerializeField] private bool _isLoop;
		[SerializeField] private bool _isRestartIfCalledAgain;

		[Space(10f)] [SerializeField] private bool _smoothFade;
		...
		private void PlaySound()
		{
			...
			if (_duration == 0)
			{
				WaitForSeconds waitForSeconds = new WaitForSeconds(_source.clip.length);
				StartState(WaitPlaySound(waitForSeconds));
			}
			else
			{
		...
		private IEnumerator WaitPlaySound(WaitForSeconds waitForSeconds)
		{
